...
def main():
    ...
    w_p = Window.partitionBy(
        F.col("actor_signature_per_project_family"),
        F.col("year"),
        F.col("month"),
        F.col("day"),
    )
    ...
    df_actor = (
        ...
        .withColumn(
            "wiki_db", F.concat(F.col("normalized_host.project"), F.lit("wiki"))
        )
    )
    ...
    df_actor = (
        df_actor.withColumn("n_p", F.sum(F.lit(1)).over(w_p))
    ...
    df_wd = (
        ...
        .where(
            F.col("snapshot") >= "2020-07-01"
        )  ## resolve issues with non-mathcing wikidata-items
    ...
    df_wd = (
        df_wd.withColumn("item_id_latest", F.first(F.col("item_id")).over(w_wd))
        .select("wiki_db", "page_id", F.col("item_id_latest").alias("item_id"))
        .drop_duplicates()
    )
    df_actor_wd = df_actor.join(df_wd, on=["page_id", "wiki_db"], how="inner")

    ## aggregate all pageviews with same actor-signature across wikis to get sessions
    df_actor_wd_agg = df_actor_wd.groupby("actor_signature_per_project_family").agg(
        F.first(F.col("access_method")).alias(
            "access_method"
        ),  ## this could change along a session
    ...
    (
        requests.map(parse_requests)
        .filter(filter_blacklist_qid)  ## remove main_page
        .filter(
            lambda x: len(x) >= nlen_min
        ...
        .flatMap(
            lambda x: sessionize(x, dt=dt)
        )  ## break sessions if interevent time is too large
        .filter(
            lambda x: len(x) >= nlen_min
        )  ## only sessions with at least length nlen_min
        .filter(
            lambda x: len(x) <= nlen_max
